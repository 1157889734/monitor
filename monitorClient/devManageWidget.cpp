    ...
    ui(new Ui::devManageWidget)
{
    ...
    ui->devStorageTableWidget->horizontalHeader()->setClickable(false); //设置表头不可点击
    ...
    ui->devStatusTableWidget->horizontalHeader()->setClickable(false); //设置表头不可点击
    ...
    if (p_ipmethod != NULL)
    {
		...
		connect(p_ipmethod, SIGNAL(enter()), this, SLOT(trainNumberSetSlot()));
		connect(p_ipmethod, SIGNAL(exit()), this, SLOT(closeKeyboardWidgetSlot()));   //关闭软键盘操作 
		connect(this, SIGNAL(enterKeyPressSignal()), p_ipmethod, SLOT(enterConfirm()));    //软键盘按enter键后响应软键盘的关闭
...
{
	DebugPrint(DEBUG_UI_NOMAL_PRINT, "delete devManageWidget\n");
	if (p_ipmethod != NULL)
	{
		delete p_ipmethod;
		p_ipmethod = NULL;
...
void devManageWidget::closeKeyboardWidgetSlot()
{ 
	ui->TrainNumberLineEdit->setText(m_TrainNumEditSave);   //软键盘直接关闭时，恢复车次输入框
	ui->TrainNumberLineEdit->setEnabled(false);
}

void devManageWidget::trainNumberSetSlot()    
{
	int i = 0, j = 0, iRet = 0;
	short year = 0;
	char acTrainNumber[128] = {0};
...
void devManageWidget::trainNumberButtonClickSlot()
{
	...
	DebugPrint(DEBUG_UI_OPTION_PRINT, "devManageWidget trainNumberSet button pressed!\n");
	
	STATE_GetCurrentUserType(acUserType, sizeof(acUserType));
	if (!strcmp(acUserType, "operator"))   //操作员不允许设置车次
	{
		DebugPrint(DEBUG_UI_MESSAGE_PRINT, "devManageWidget this user type has no right to set train number!\n");
		QMessageBox box(QMessageBox::Warning,QString::fromUtf8("提示"),QString::fromUtf8("该类型用户无此权限!"));     //提示框
	    box.setStandardButtons (QMessageBox::Ok);   
	    box.setButtonText (QMessageBox::Ok,QString::fromUtf8("确 定"));     
	    box.exec();
		return;
