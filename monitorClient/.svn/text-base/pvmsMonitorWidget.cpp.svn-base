...
void *monitorThread(void *param)     //实时监控线程，对通道轮询、全屏、预置点返回、设备状态等进行循环监控
{
	...
	DebugPrint(DEBUG_UI_NOMAL_PRINT, "pvmsMonitorWidget monitor thread begin !\n");
    ...
    {  		
		...
		/*if (1 == pvmsMonitorPage->m_iSystimeChangeFlag)     //系统时间改变，所有计时重新开始
		{
			pvmsMonitorPage->m_iSystimeChangeFlag = 0;
			tPollingCurTime = time(NULL);
			pvmsMonitorPage->tPollingOparateTime = tPollingCurTime;			
			tPresetReturnCurTime = time(NULL);
			for (i = 0; i < pvmsMonitorPage->m_iCameraNum; i++)
			{
				pvmsMonitorPage->m_tCameraInfo[i].tPtzOprateTime = tPresetReturnCurTime;
			}
			tFullScreenCurTime = time(NULL);
			pvmsMonitorPage->m_lastActionTime = tFullScreenCurTime;
			tGetDevStateCurTime = time(NULL);
			tGetDevStateOldTime = tGetDevStateCurTime;
			tSetTimeCurTime = time(NULL);
			tSetTimeOldTime = tSetTimeCurTime; 
		}*/
	...
	DebugPrint(DEBUG_UI_NOMAL_PRINT, "pvmsMonitorWidget monitor thread end!\n");
    return NULL;
}
...
void pvmsMonitorWidget::cmpOptionCtrlSlot(int iType, int iCh)
{
	...
	{
		cmpHandle = CMP_CreateMedia(m_playWin->winId());   //创建媒体播放窗口
		if (NULL ==cmpHandle)  
		{
			DebugPrint(DEBUG_UI_ERROR_PRINT, "[%s] CMP_CreateMedia %d error\n",__FUNCTION__,iCh); 
			return;
		}
	    ...
	    if (iRet != 0)    
	    {			
			DebugPrint(DEBUG_UI_ERROR_PRINT, "[%s] CMP_OpenMediaPreview %d error\n",__FUNCTION__,iCh);
			return;
	    }	    
	...
	else if (CMP_CMD_DESTORY_CH == iType)
	{
	    ...
	    if (iRet != 0)    
	    {
			DebugPrint(DEBUG_UI_ERROR_PRINT, "[%s] CMP_CloseMedia error!iRet=%d, cameraNo=%d\n",__FUNCTION__,iRet, iCh);
			return;
	    } 	
	...
	else if (CMP_CMD_ENABLE_CH == iType)
	{
		...
		if (iRet != 0)    
	    {
			DebugPrint(DEBUG_UI_ERROR_PRINT, "[%s] CMP_SetWndDisplayEnable on error!iRet=%d, cameraNo=%d\n",__FUNCTION__,iRet, iCh);
			return;
	    }
	}
	else if (CMP_CMD_DISABLE_CH == iType)
	{
		...
		if (iRet != 0)    
	    {
			DebugPrint(DEBUG_UI_ERROR_PRINT, "[%s] CMP_SetWndDisplayEnable off error!iRet=%d, cameraNo=%d\n",__FUNCTION__,iRet, iCh);
			return;
	    }
	}
    ...
    ui(new Ui::pvmsMonitorWidget)
{
    ...
    ui->setupUi(this);
    this->setWindowFlags(Qt::FramelessWindowHint);   //设置窗体无标题栏
	...
	g_buttonGroup = new QButtonGroup();      //预置点编号按钮组成一组，方便批量处理，这里对按钮进行了编号，按钮编号和预置点实际编号设置相同，方便处理
...
{
	DebugPrint(DEBUG_UI_NOMAL_PRINT, "delete pvmsMonitorWidget\n");
...
{	
	...
	DebugPrint(DEBUG_UI_NOMAL_PRINT, "[%s] create a play window and set full screen\n",__FUNCTION__);
	...
	/*获取车型配置信息，得到所有摄像头信息，逐个请求RTSP流，默认是都不显示的，在monitorThread线程里处理使能达到轮询效果*/
	...
	DebugPrint(DEBUG_UI_NOMAL_PRINT, "[%s] get all normal camera num is %d\n",__FUNCTION__,m_iCameraNum);
	...
	m_threadId = 0;
	m_iThreadRunFlag = 1;  
	DebugPrint(DEBUG_UI_NOMAL_PRINT, "[%s] create monitorThread\n",__FUNCTION__);
...
void pvmsMonitorWidget::startPollingSlot()  //开始按钮按键信号响应槽函数，设置轮询使能
{	
	DebugPrint(DEBUG_UI_OPTION_PRINT, "pvmsMonitorWidget start polling!\n");
    m_iPollingFlag = 1;
...
void pvmsMonitorWidget::pausePollingSlot()  //暂停按钮按键信号响应槽函数，设置轮询使能
{	
	DebugPrint(DEBUG_UI_OPTION_PRINT, "pvmsMonitorWidget pause polling!\n");
    m_iPollingFlag = 0;
...
{
	int i = 0;
	DebugPrint(DEBUG_UI_OPTION_PRINT, "pvmsMonitorWidget preset %d button pressed!\n",iButtonNo);
...
{	
	...
	DebugPrint(DEBUG_UI_OPTION_PRINT, "pvmsMonitorWidget preset set button pressed!\n");
	#if 0
	if (1 == m_iSelectPresetNo)   //预置点1设置弹框提示不允许
	{
		DebugPrint(DEBUG_UI_MESSAGE_PRINT, "pvmsMonitorWidget can't set preserNo of 1!\n");
		QMessageBox box(QMessageBox::Warning,QString::fromUtf8("错误"),QString::fromUtf8("不能对预置点1进行设置!"));     //新建消息提示框，提示错误信息
...
void pvmsMonitorWidget::manualSwitchLastCameraSlot() //轮询播放上一个摄像头的按钮被按下，更新摄像机切换状态为切换到上一个的状态
{
	DebugPrint(DEBUG_UI_OPTION_PRINT, "pvmsMonitorWidget manualSwitchLastCamera button pressed!\n");
...
void pvmsMonitorWidget::manualSwitchNextCameraSlot() //轮询播放下一个摄像头的按钮被按下，更新摄像机切换状态为切换到下一个的状态
{
	DebugPrint(DEBUG_UI_OPTION_PRINT, "pvmsMonitorWidget manualSwitchNextCamera button pressed!\n");
...
{
	DebugPrint(DEBUG_UI_OPTION_PRINT, "pvmsMonitorWidget close PresetPasswdPage!\n");
...
void pvmsMonitorWidget::setPresetSlot()    
{
	...
	DebugPrint(DEBUG_UI_NOMAL_PRINT, "[%s] delelte presetPasswdConfirmPage!\n", __FUNCTION__);
...
void pvmsMonitorWidget::closePlayWin()
{
	...
	DebugPrint(DEBUG_UI_OPTION_PRINT, "pvmsMonitorWidget close play window!\n");
	...
	m_iCameraNum = 0;
	DebugPrint(DEBUG_UI_NOMAL_PRINT, "[%s] destory all camera media success !\n", __FUNCTION__);
