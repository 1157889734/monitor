...
#include "ui_commonKeyboardWidget.h"

int g_iShiftBtClickNum = 0;   //记录键盘shift键的按键次数，以便奇数次和偶数次按键做出不同的相应

commonKeyboardWidget::commonKeyboardWidget(commonInputMethod *ipm, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::InputWidget)
{
    ...
    connect(this, SIGNAL(sendCloseMsg()), ipm, SLOT(closeWidget()));
}
commonKeyboardWidget::~commonKeyboardWidget()
{
    delete ui;
}

void commonKeyboardWidget::buttonClick()
{
...
void commonKeyboardWidget::setShiftFlag()
{
        g_iShiftBtClickNum++;
        if (0 == g_iShiftBtClickNum%2)
        {
            m_capsFlag = 0;
        }
        else
